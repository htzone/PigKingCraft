local MAX_SHIELD_TIME = 10 --加入最大的躲藏时间
...
UseShield = Class(BehaviourNode, function(self, inst, damageforshield, shieldtime, hidefromprojectiles)
	...
	self.ExitTask = nil
	self.inst.isShieldShouldExit = false
...
function UseShield:Visit()
	...
	if self.status == READY  then
		if self:ShouldShield() or self.inst.sg:HasStateTag("shield") then
			...
			self.ExitTask = self.inst:DoTaskInTime(MAX_SHIELD_TIME, function()
				self.inst.isShieldShouldExit = true
			end)
	...
	if self.status == RUNNING then
		if not self.inst.isShieldShouldExit and
		(not self:TimeToEmerge() or self.inst.components.health.takingfiredamage) then 
			self.status = RUNNING
		else
