...
function TravelScreen:LoadDests()
    local info_pack = self.attach.replica.pkc_travelable and self.attach.replica.pkc_travelable:GetDestInfos()
    self.dest_infos = {}
...
function TravelScreen:Travel(index)
    ...
    local pkc_travelable = self.attach.replica.pkc_travelable
    if pkc_travelable then
        pkc_travelable:Travel(self.owner, index)
    end
...
function TravelScreen:OnCancel()
    ...
    local pkc_travelable = self.attach.replica.pkc_travelable
    if pkc_travelable then
        pkc_travelable:Travel(self.owner, nil)
    end
