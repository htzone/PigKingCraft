...
local Travelable =
    ...
    function(self, inst)
        ...
        self._infos = net_string(inst.GUID, "pkc_travelable._infos")

        self.screen = nil
        self.opentask = nil

        if TheWorld.ismastersim then
            self.classified = SpawnPrefab("pkc_travelable_classified")
            self.classified.entity:SetParent(inst.entity)
        else
            if self.classified == nil and inst.pkc_travelable_classified ~= nil then
                self.classified = inst.pkc_travelable_classified
                inst.pkc_travelable_classified.OnRemoveEntity = nil
                inst.pkc_travelable_classified = nil
...
function Travelable:BeginTravel(traveller)
    if self.inst.components.pkc_travelable ~= nil then
        if self.opentask ~= nil then
            self.opentask:Cancel()
            self.opentask = nil
        end
        self.inst.components.pkc_travelable:BeginTravel(traveller)
...
function Travelable:Travel(traveller, index)
    if self.inst.components.pkc_travelable ~= nil then
        self.inst.components.pkc_travelable:Travel(traveller, index)
...
function Travelable:EndTravel()
    ...
    if self.inst.components.pkc_travelable ~= nil then
        self.inst.components.pkc_travelable:EndTravel()
...
function Travelable:SetTraveller(traveller)
    ...
    if self.inst.components.pkc_travelable == nil then
        --Should only reach here during pkc_travelable construction
